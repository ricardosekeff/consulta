...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        $api_user_variable = $this->config->item('user_key_name');

        // Work out the name of the SERVER entry based on config
        $key_name = 'HTTP_' . strtoupper(str_replace('-', '_', $api_key_variable));
        $user_name = 'HTTP_' . strtoupper(str_replace('-', '_', $api_user_variable));
        ...
        if (
            ...
            ($user_id = isset($this->_args[$api_user_variable]) ? $this->_args[$api_user_variable] : $this->input->server($user_name))
            )
        {
            if ( ! ($row = $this->rest->db->where(array($this->config->item('rest_key_column') => $key, 'user_id' => $user_id))->get($this->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }
     ...
     * @param NULL $sanitize Sanitization of the field
     * @return array|string|NULL Value from the GET request; otherwise, NULL
     */
    public function get($key = NULL, $xss_clean = NULL, $sanitize = NULL)
    {
        if ($key === NULL)
        {
        ...
        switch ($sanitize) {
            case 'alphanum':
                return isset($this->_get_args[$key]) ? preg_replace('/[^a-zA-Z0-9\s]+/', '', $this->_get_args[$key]) : NULL;
            case 'alpha':
                return isset($this->_get_args[$key]) ? preg_replace('/[^a-zA-Z\s]+/', '', $this->_get_args[$key]) : NULL;
            case 'num':
                return isset($this->_get_args[$key]) ? preg_replace('/[^\d\s]+/', '', $this->_get_args[$key]) : NULL;
            case 'date':
                return isset($this->_get_args[$key]) ? preg_replace('/[^\d-]+/', '', $this->_get_args[$key]) : NULL;
            default: 
